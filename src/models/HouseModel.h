...
public:
	HouseModel() : Model() {
		...
		if (err != GL_NO_ERROR) {
			std::cout << "STARTGL ERROR: " << err << std::endl;
		}
		...
		const aiScene* scene = importer.ReadFile("./src/models/definitions/house/house.obj", importOptions);
		...
		if (scene) { //pokud bylo nacteni uspesne
			...
			printf("Error during parsing mesh from %s : %s \n", "houseobj", importer.GetErrorString());
		}
		glBindVertexArray(0);

		glActiveTexture(GL_TEXTURE0);
		texture = SOIL_load_OGL_texture("./src/models/definitions/house/house.png", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
		if (texture == NULL) {
